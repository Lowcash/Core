...
class IndicatorParser {
 public:
   ...
   static void GetIchimokuValues(Ichimoku &p_OutIchimoku[], IchimokuSettings &p_IchimokuSettings, const int p_From, const int p_To);
};
...
void IndicatorParser::GetIchimokuValues(Ichimoku &p_OutIchimoku[], IchimokuSettings &p_IchimokuSettings, const int p_From, const int p_To) {
   if(ArrayResize(p_OutIchimoku, MathAbs(p_From - p_To) + 1) != -1) {
      for(int i = p_From, MAIdx = 0; (p_From <= p_To) ? i <= p_To : i >= p_To ; p_From <= p_To ? ++i : --i, MAIdx++) {
         SetIchimoku(&p_IchimokuSettings, i, p_OutIchimoku[MAIdx].TenkanSen, p_OutIchimoku[MAIdx].KijunSen, p_OutIchimoku[MAIdx].ChinkouSpan, p_OutIchimoku[MAIdx].SenkouSpanA, p_OutIchimoku[MAIdx].SenkouSpanB);
      }
   } else {
